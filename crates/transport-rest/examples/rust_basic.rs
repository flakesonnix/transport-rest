...
#![allow(clippy::result_large_err)] // matches the crate-wide policy
//!
...
async fn main() -> Result<(), transport_rest::TransportRestError> {
    ...
    for location in &locations {
        println!(
            "found: {} ({})",
            location.name().unwrap_or("<unnamed>"),
            location.id().unwrap_or("?")
        );
    }

    // 2) Departure board for the first stop-like result.
    if let Some(id) = locations
        .iter()
        .find_map(|l| l.as_stop().map(|s| s.id.clone()))
    {
    ...
    let journeys = client
        .journeys(
            JourneyPlace::StopId("8011160".into()),
            JourneyPlace::StopId("8000108".into()),
        )
        .results(3)
        .get()
        .await?;
